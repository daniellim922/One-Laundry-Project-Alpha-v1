...
5. Run schema workflows individually with `npm run db:migrate` (applies `db/schema.ts` via `drizzle-kit push`), `npm run db:seed`, or `npm run db:wipe`.
...
8. Set `NEXT_PUBLIC_SUPABASE_URL`, `NEXT_PUBLIC_SUPABASE_ANON_KEY`, and `SUPABASE_SERVICE_ROLE_KEY` in `.env` (use values from `npm run sb:status`).
9. In Supabase Studio → **Authentication → Users**, create a user with email and password (self-service signup is disabled in `supabase/config.toml`; see `.env.example`).
10. Run the app with `npm run dev`.

`npm run db:reset` wipes, pushes schema, and seeds the database so the deterministic historical payroll dataset is ready for app use and test flows. Start the stack with `npm run sb:start` first. Seed only writes Postgres data; it does not create Supabase Auth users.

The app, `drizzle-kit push`, wipe, and seed all use **`DATABASE_URL`**.

Runtime sign-in uses Supabase email and password. Typical local variables:
...
Local auth is wired for password sign-in:

- Supabase Auth is enabled in `supabase/config.toml`.
- Self-service signup stays disabled locally; create users in Studio (or via the Auth Admin API).
- Local mail capture (`[inbucket]` config, surfaced as `Mailpit` by the current Supabase CLI) is available at `http://127.0.0.1:54324` for future email flows (for example password recovery).
- Redirects are allowlisted for both `http://127.0.0.1:3000` and `http://localhost:3000`, including `/auth/callback`, for future Supabase redirect-based flows.
...
2. Run `npm run db:reset` if you need a fresh seeded database.
3. Ensure a user exists in Supabase Auth with a known password (Studio or Auth API).
4. Start the app with `npm run dev` and sign in at `/login` with that email and password.
