...
import { createMay2026AttendRecordArtifactsFromLatestWorkerHandoff } from "./attendrecord-artifact-helpers";
import {
...
test.describe("Timesheet userflow", () => {
    test("imports the deterministic May 2026 AttendRecord workbook from the persisted worker handoff", async ({
        page,
    }) => {
        const artifact =
            await createMay2026AttendRecordArtifactsFromLatestWorkerHandoff();

        expect(artifact.expectedRowCount).toBe(114);
