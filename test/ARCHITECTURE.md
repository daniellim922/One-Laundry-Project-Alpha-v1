...
- **Worker E2E fixture** — In this doc, that phrase means **a Worker + Employment row created only for Playwright flows** (e.g. names prefixed like `E2E Worker <timestamp>`), not a new domain concept. Worker setup and form helpers now live inline inside each `test/playwright/workers/*.spec.ts` file.
- **Worker matrix + downstream flows** — Matrix identities live in `test/playwright/workers/workers.json`; created Workers are persisted by name/profile for chained specs in `test/playwright/workers/.matrix-e2e-state.json` (written by `worker-create.spec.ts`). Playwright runs a serialized project chain: `worker-create` → `worker-read` → `worker-delete` → `worker-update` → `timesheet-create` → `timesheet-read` → `timesheet-update` → `timesheet-delete` → `advance-create` → `advance-read` → `advance-update` (see `playwright.config.ts`). Those `worker-*` / `timesheet-*` / `advance-*` specs are excluded from the generic `chromium` project to avoid double execution. Specs are intentionally self-contained (inline Playwright helper functions per spec). The only shared helper module is `test/playwright/shared/matrix.ts` for matrix JSON state and date helpers; overview smoke specs remain `timesheets/timesheet-00-overview.spec.ts` and `advances/advance-00-overview.spec.ts`.
