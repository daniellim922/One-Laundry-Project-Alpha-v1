...
- **Auth entry flow.** `/` remains the marketing landing page, `/login` is the public magic-link sign-in boundary, `/auth/callback` completes the Supabase auth exchange, and `/dashboard` redirects unauthenticated requests into `/login` through `proxy.ts`.
