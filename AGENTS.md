...
- **Guided monthly workflow activity** is month-scoped shared operational guidance keyed by the business timezone month (`Asia/Singapore`). Successful minimum-hours bulk updates emit activity signals; the dashboard snapshot reads those signals and derives `done/current/up next`.
