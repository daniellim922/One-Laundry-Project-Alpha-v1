...
Next.js 16 (App Router, React 19, React Compiler) · TypeScript 5 · PostgreSQL + Drizzle ORM · shadcn/ui (new-york) · Tailwind CSS v4 · TanStack React Table v8 · react-hook-form + Zod · Recharts · Vitest. Payroll and advance voucher PDFs use `@react-pdf/renderer` on the client, upload to Supabase Storage (`documents` bucket), and persist `pdfStoragePath` on each record; downloads stream from storage via API routes (see **PDF exports** below).
